...
const registerUser = async (req, res) => {
  ...
  const validatePayload = validator.isValidPayload(payload, commandModel.registerUser);
  const postRequest = async (result) => {
    if (result.err) {
      return result;
    }
  ...
  const sendResponse = async (result) => {
    (result.err) ? wrapper.response(res, 'fail', result, 'Register user fail')
...
const verifyOtpRegister = async (req, res) => {
  ...
  const sendResponse = async (result) => {
    (result.err) ? wrapper.response(res, 'fail', result, 'Verify otp register user fail')
      : wrapper.response(res, 'success', result, 'Verify otp register user success', http.OK);
  };
  sendResponse(await postRequest(validatePayload));
};
