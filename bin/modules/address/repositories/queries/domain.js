...
class Address {
  ...
  async getAllAddress() {
    const ctx = 'getAllAddress';
    const address = await this.query.findAllAddress({});
    if (address.err) {
      logger.error(ctx, 'error', 'Can not find all address', address.err);
      return wrapper.error(new NotFoundError('Can not find all address'));
    }
    const { data } = address;
